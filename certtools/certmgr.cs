...
namespace Mono.Tools {
    ...
    class CertificateManager {
        ...
        {
            Console.WriteLine(
                "Mono Certificate Manager - version 3.12.0 - Codice Software custom");
            Console.WriteLine(
        ...
        static private void Help()
        {
            ...
            Console.WriteLine ("\t-y\t\tSSL: batch mode. Accept certificates automatically.");
        ...
        static void Ssl (string host, bool machine, bool verbose, bool batch)
        {
            if (verbose) {
            ...
            if (coll == null) {
                Console.WriteLine();
                Console.WriteLine ("Unable to retrieve certificates from {0}.", host);
                return;
            }
            ...
            for (int i = coll.Count - 1; i >= 0; i--)
            {
                ...
                bool selfsign = false;
                bool failed = !TryIsSelfSigned(x509, out selfsign);

                store = GetCertificateStore(machine, selfsign, i == 0);

                LogCertificateData(x509, selfsign);

                if ((i > 0) && !selfsign)
                {
                    try
                    {
                        if (!IsSignatureValid(x509, coll[i-1]))
                        {
                            Console.WriteLine(
                                "   *** WARNING: Certificate signature is INVALID ***");
                            if (batch)
                            {
                                Console.WriteLine("   *** ERROR: Invalid signature detected in batch mode. Aborting.");
                                break;
                            }
                        }
                    }
                ...
                if (!ImportCertificate(x509, store, batch, verbose))
                    break;
                n++;
            }
            ...
            Console.WriteLine();
            if (n == 0) {
                Console.WriteLine ("No certificate was added to the stores.");
                return;
            }
        ...
        static bool AskForManualCertificateImportUserConfirmation(string storeName)
        {
            Console.Write("Import this certificate into the {0} store ?", storeName);
            string answer = Console.ReadLine().ToUpper();
            return (answer == "YES") || (answer == "Y");
        }

        static bool TryIsSelfSigned(X509Certificate x509, out bool isSelfSigned)
        {
            isSelfSigned = false;
            try
            {
        ...
        static X509Store GetCertificateStore(
            bool machine, bool isSelfSigned, bool isTargetServer)
        {
            if (isSelfSigned) {
                // this is a root
                return GetStoreFromName(X509Stores.Names.TrustedRoot, machine);
            }

            if (isTargetServer)
            {
                // server certificate isn't (generally) an intermediate CA
                return GetStoreFromName(X509Stores.Names.OtherPeople, machine);
            }

            // all other certificates should be intermediate CA
            return GetStoreFromName(X509Stores.Names.IntermediateCA, machine);
        }

        static void LogCertificateData(X509Certificate x509, bool isSelfSigned)
        {
        ...
        static bool IsSignatureValid(X509Certificate signer, X509Certificate signed)
        {
            if (signer.RSA != null)
                return signed.VerifySignature(signer.RSA);
            if (signer.DSA != null)
                return signed.VerifySignature(signer.DSA);
        ...
        static bool ImportCertificate(
            X509Certificate x509, X509Store store, bool isBatchMode, bool verbose)
        {
            if (store.Certificates.Contains(x509))
            {
                Console.WriteLine(
                    "This certificate is already in the {0} store.", store.Name);
                return true;
            }

            if (isBatchMode || AskForManualCertificateImportUserConfirmation(store.Name))
            {
                store.Import(x509);
                return true;
            }
        ...
        static void Main (string[] args)
        {
            ...
            try {
                ...
                case Action.List:
                    ...
                    break;
                case Action.Ssl:
                    Ssl (file, machine, verbose, batch);
                    break;
